  ...
  while (e != list_end (&sleeping_list))
    {
	  ...
	  if (t->ticks_until_wake_up > 0)
        {
          t->ticks_until_wake_up--;
	      if (t->ticks_until_wake_up <= 0)
		    {
			  thread_unblock(t);
			  list_remove (temp);
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  t->ticks_until_wake_up = 0;
  t->real_priority = priority;
  t->lock_curr = NULL;
  list_init (&t->locks_held_list);
...
{
  struct thread *cur = thread_current ();
  cur->ticks_until_wake_up = ticks;
  list_push_back (&sleeping_list, &cur->sleepingelem);
  thread_block ();
}
...
thread_update_priority (struct thread *t)
{
  ...
  if (list_empty (&t->locks_held_list))
    t->priority = real_priority;
  else
    {
	  int lock_priority = list_entry (list_max (&t->locks_held_list,
