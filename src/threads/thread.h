...
struct thread
  {
	...
	/* Owned by thread.c. */
	struct list_elem sleepingelem;         /* List element for sleeping
	                                       threads. */
	int64_t ticks_until_wake_up;  /* Ticks remaining from waking up. */
	
	/* Owned by thread.c and synch.c. */
	int real_priority;                  /* Real priority while being
	                                       donated. */
	struct list locks_held_list;             /* All locks held. */
	struct lock *lock_curr;          /* The thread been locked by this
	                                       lock. */
